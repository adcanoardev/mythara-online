...
const BADGE_INFO = [
    {icon:"🪨",name:"Medalla Roca",level:10},
    {icon:"💧",name:"Medalla Cascada",level:15},
    {icon:"⚡",name:"Medalla Trueno",level:20},
    {icon:"🌿",name:"Medalla Arcoíris",level:25},
    {icon:"☠️",name:"Medalla Alma",level:30},
    {icon:"🔮",name:"Medalla Marsh",level:35},
    {icon:"🔥",name:"Medalla Volcán",level:40},
    {icon:"🌍",name:"Medalla Tierra",level:50},
];
...
export default function PerfilPage() {
    ...
    useEffect(() => {
        Promise.all([api.trainer(), api.pokemon()])
            .then(([t, p]) => { setTrainer(t); setPokemon(p); });
    }, []);
    ...
    const party = pokemon.filter(p => p.isInParty);
    ...
    return (
        <Layout sidebar={<TrainerSidebar />}>
            ...
            <div className="flex-shrink-0 px-6 py-4 border-b border-border relative overflow-hidden"
                ...
                <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 60% 100% at 90% 50%, rgba(123,47,255,0.15) 0%, transparent 60%)" }}/>
                <div className="relative flex items-center gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 max-w-xs">
                            <div className="bg-white/5 rounded-full h-1.5 overflow-hidden mb-0.5">
                                <div className="h-full rounded-full" style={{ width:`${xpPct}%`, background:"linear-gradient(90deg,#4cc9f0,#7b2fff)" }}/>
                            </div>
                    ...
                    <div className="flex gap-6 text-center flex-shrink-0">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-muted">Medallas</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 flex gap-4 p-6 overflow-hidden">
                ...
                <div className="flex-1 bg-card border border-border rounded-2xl p-5 flex flex-col overflow-hidden">
                    <div className="font-display font-bold text-lg tracking-widest mb-4 flex-shrink-0">🏅 Medallas</div>
                    <div className="grid grid-cols-4 gap-4 flex-1 content-center">
                        {BADGE_INFO.map((badge, i) => {
                            const earned = trainer?.medals?.includes(i);
                            return (
                                <div key={i} className="text-center group relative">
                                    <div className={`text-4xl transition-all ${earned ? "" : "grayscale opacity-25"}`}>{badge.icon}</div>
                                    <div className="text-xs mt-1 font-display" style={{ color: earned ? "#ffd60a" : "#5a6a85" }}>
                                        {earned ? badge.name.split(" ")[1] : `Nv.${badge.level}`}
                                    </div>
                                </div>
                            );
                        })}
                        ...
                        <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
                            {party.map((p: any) => (
                                <div key={p.id} className="flex items-center gap-3 bg-bg3 rounded-xl p-3">
                                    <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${p.pokedexId}.png`}
                                        className="w-10 h-10 flex-shrink-0" style={{ imageRendering:"pixelated" }} alt=""/>
                                    <div className="flex-1 min-w-0">
                                        <div className="font-display font-bold text-sm truncate">#{p.pokedexId}</div>
